...
// Button state for MUTE combinations
bool muteButtonHeld = false;
...
void updateModeLEDs() {
    ...
    // MUTE LED shows if it's being held
    LedColor muteColor = muteButtonHeld ? ControllerAPI::GREEN() : ControllerAPI::OFF();
...
void playNoteForStep(int channelIndex, int stepIndex) {
    if (channels[channelIndex].isMuted()) return;
...
void handleControlChange(byte channel, byte control, byte value) {
    ...
    if (control == CC_DEVICE && value > 0) {
        // Toggle between homepage and channel mode
        currentMode = (currentMode == MODE_HOMEPAGE) ? MODE_CHANNEL : MODE_HOMEPAGE;
        Serial.print("Mode: ");
        Serial.println(currentMode == MODE_HOMEPAGE ? "HOMEPAGE" : "CHANNEL");
        updateLEDs();
    }
    else if (control == CC_MUTE) {
        muteButtonHeld = (value > 0);
        updateModeLEDs();
    }
    // Handle mode-specific controls
        ...
        // Check for MUTE + button combinations
        if (muteButtonHeld && control >= TRACK_FOCUS_START_CC && control < TRACK_FOCUS_START_CC + 8 && value > 0) {
            int targetChannel = control - TRACK_FOCUS_START_CC;
            switchToChannel(targetChannel);
            return; // Don't process as step toggle
        }
    ...
    // Transport controls work in both modes
...
void setup() {
    Serial.begin(115200);
    Serial.println("Multi-Channel 16-Step Sequencer");
    ...
    Serial.println("- Channel view: MUTE + TRACK FOCUS to switch channels");
    Serial.println("- SEND A knobs: Set notes for steps 1-8");
    Serial.println("- SEND B knobs: Set notes for steps 9-16");
    Serial.println("- Transport and pattern controls work as before");
